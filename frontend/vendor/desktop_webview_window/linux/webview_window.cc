...
WebviewWindow::WebviewWindow(FlMethodChannel *method_channel, int64_t window_id,
  ...
  // [FOCSQ] use-after-free: erase() колбэка удаляет WebviewWindow из map
  // (деструктор освобождает память), а код ПОСЛЕ него читает поля этого
  // объекта — window_id_/method_channel_ уже в освобождённой памяти
  // (первые слова чанка затираются tcache) и вызов уходит по болтающемуся
  // указателю. Крашил приложение при каждом закрытии WebView-окна
  // (капча крестиком, вход ВК после программного close()).
  // Копируем нужное ДО разрушения, потом только используем.
  g_signal_connect(G_OBJECT(window_), "destroy",
                   G_CALLBACK(+[](GtkWidget *, gpointer arg) {
                     ...
                     fl_method_channel_invoke_method(
                         method_channel, "onWindowClose", args, nullptr,
                         nullptr, nullptr);
                     g_object_unref(method_channel);
                   }),
                   this);
  gtk_window_set_title(GTK_WINDOW(window_), title.c_str());
  // [FOCSQ] Иконка окна из бандла главного приложения: окно WebView
  // создаётся без иконки, и на системах со сломанным SVG-загрузчиком
  // (glycin/bwrap на живой Manjaro) фолбэк image-missing.svg из темы
  // ронял GTK assertion'ом при realize. Файл напрямую не ходит в
  // икон-тему. WebView-окно живёт в процессе приложения, поэтому
  // /proc/self/exe — сам бинарник focsq в корне бандла.
  {
    char exe_path[4096];
    ...
    if (length > 0) {
      ...
      const gchar *sizes[] = {"256x256", "128x128", "64x64",
                             "48x48", "32x32", "16x16"};
      for (gulong i = 0; i < G_N_ELEMENTS(sizes); i++) {
        gchar *file = g_build_filename(bundle, "data", "icons", "hicolor",
                                       sizes[i], "apps", "focsq.png", nullptr);
        const gboolean loaded = gtk_window_set_icon_from_file(
            GTK_WINDOW(window_), file, nullptr);
        g_free(file);
        if (loaded) {
          break;
        }
  ...
  // [FOCSQ] Шапка живёт на ВТОРОМ Flutter-движке (свой FlView+изолят).
  // Наши окна (вход ВК, капча) открываются с titleBarHeight 0 — шапки
  // не видно, а движок всё равно поднимался целиком. На Flutter 3.44
  // его teardown ломается: fl_view_dispose зовёт fl_engine_remove_view,
  // embedder отказывает («The implicit view cannot be removed»),
  // движок-зомби рвёт общий EGLDisplay (eglMakeCurrent failed ×5), и
  // GDK рисует DAMAGE-событие уже разрушенного окна — SIGSEGV в
  // gdk_window_end_draw_frame (два керндампа 2026-09-05: при
  // самозакрытии окна капчи по таймауту и при закрытии приложения).
  // Без второго движка teardown чистый, поэтому при нулевой высоте
  // FlView не создаём вовсе.
...
WebviewWindow::~WebviewWindow() {
  // [FOCSQ] Деструктор ВСЕГДА вызывается изнутри destroy-обработчика
  // GTK-окна: on_close_callback_ стирает объект из map — единственный
  // путь сюда. То есть window_ уже в процессе разрушения, и повторный
  // gtk_widget_destroy(window_) здесь — реентерабельный
  // g_object_run_dispose изнутри чужого dispose: главный цикл GTK
  // на этом зависал намертво при каждом закрытии окна ВК (крестиком
  // или программно после перехвата токена). До-уничтожать ничего не
  // нужно: начальный ref toplevel-окна освобождается его destroy.
  if (webview_ != nullptr) {
