...
/// Дамп трафика капчи в файл: включается env-переменной CSQTT_CAPTCHA_DEBUG
/// (значение = путь к файлу, либо "1" -> captcha_debug.log рядом с запуском).
// [FOCSQ] Изменено относительно оригинала csqtt: добавлен отладочный дамп
// ответов капчи в файл (включается env CSQTT_CAPTCHA_DEBUG, по умолчанию выключен).
fn debug_dump(tag: &str, text: &str) {
    let Some(raw) = std::env::var_os("CSQTT_CAPTCHA_DEBUG") else {
        return;
    };
    use std::io::Write as _;
    let path = if raw == "1" {
        std::path::PathBuf::from("captcha_debug.log")
    } else {
...
impl CaptchaSolver {
            ...
            "rjs" => {
                    ...
                    .await
                {
                    ...
                    Err(error) => {
                        ...
                        self.webview(stream_id, captcha, "auto", Duration::from_secs(45))
                            .await
                    }
                }
            }
    ...
    async fn solve_auto(
        ...
        // [FOCSQ] Короткая цепочка: каждая попытка Rust прожигает попытку
        // сессии у VK — длинная цепочка приводила к «Лимит попыток исчерпан»
        // уже в первом окне WebView. Одна быстрая Rust-попытка, затем окно.
        match self
            ...
            .await
        {
            ...
            Err(error) => {
                crate::log_error!(
                    "[STREAM {stream_id}] [КАПЧА] AUTO: Rust v2 не решил: {error} · открываю окно"
                );
            }
        }
        crate::log_error!(
            "[STREAM {stream_id}] [КАПЧА] AUTO: WBV Auto (timeout 45s, можно решить вручную)"
        );
        match self
            .webview(stream_id, captcha, "auto", Duration::from_secs(45))
            .await
        {
            Ok(token) => {
                crate::log_error!("[STREAM {stream_id}] [КАПЧА] AUTO: WBV решил капчу");
                return Ok(token);
            }
            Err(error) => {
                crate::log_error!(
                    "[STREAM {stream_id}] [КАПЧА] AUTO: WBV Auto ошибка: {error}"
                );
            }
        }
        crate::log_error!(
            "[STREAM {stream_id}] [КАПЧА] AUTO: повторное окно (ручной режим, 60s)"
        );
        self.webview(stream_id, captcha, "manual", Duration::from_secs(60))
            .await
            .with_context(|| "automatic captcha chain failed; manual fallback failed")
    }
...
impl CaptchaInit {
    fn to_debug_json(&self) -> String {
        let settings: Vec<String> = self
            ...
            .captcha_settings
            .iter()
            .map(|setting| format!("{}={}", setting.kind, setting.settings))
            .collect();
        format!(
            "show_captcha_type={:?} settings={:?}",
            self.data.show_captcha_type, settings
        )
    }
}
...
impl CaptchaSession<'_> {
    async fn solve_once(&self, captcha: &VkCaptchaError) -> Result<String> {
        ...
        if let Some(init) = &page.init {
            debug_dump("PARSED window.init", &init.to_debug_json());
        }
        debug_dump(
            "PARSED page",
            &format!(
                "pow_difficulty={}, pow_input={}, script_url={}, debug_info={}",
                page.pow_difficulty,
                page.pow_input,
                page.script_url,
                page.debug_info
        ...
        if page.pow_input.is_empty() {
            // [FOCSQ] PoW-настроек нет в HTML — спрашиваем у initSession:
            // он отдаёт content_settings, включая type="pow". Раньше здесь
            // был немедленный bail, из-за него сыпалось
            // «failed to find PoW settings».
                ...
                .await
            {
                Ok(value) => {
                    if let Some(settings) = value
                        .get("response")
                        .and_then(|response| response.get("content_settings"))
                        .and_then(Value::as_array)
                    {
                        for setting in settings {
                            if string_value(setting.get("type")) == "pow" {
                                page.pow_input = string_value(setting.get("settings"));
                                if page.pow_input.is_empty() {
                                    page.pow_input = string_value(setting.get("settings_key"));
                                }
                                if !page.pow_input.is_empty() && page.pow_difficulty == 0 {
                                    page.pow_difficulty = 4;
                                }
                            }
                        }
                    }
                ...
                Err(error) => {
                    crate::log_error!("[КАПЧА] initSession (PoW fallback) ошибка: {error}")
                }
            }
        }
        if page.pow_input.is_empty() {
    ...
    async fn raw(
        ...
        let mut request = self.client.request(method.clone(), endpoint).headers(headers);
        if let Some(values) = form {
            request = request
                .header(header::CONTENT_TYPE, "application/x-www-form-urlencoded")
                .body(encode_form(values));
            debug_dump(
                &format!("REQ {method} {endpoint}"),
                &encode_form(values),
            );
        } else {
            debug_dump(&format!("REQ {method} {endpoint}"), "(no body)");
        }
        let response = request.send().await?;
        let body = response.bytes().await?;
        debug_dump(
            &format!("RES {method} {endpoint}"),
    ...
    async fn solve_checkbox(
        ...
        // [FOCSQ] Живая траектория курсора вместо пустого "[]": иначе
        // checkbox стабильно возвращал BOT.
        let cursor = crate::captcha_slider::build_checkbox_cursor();
        self.request(
        ...
        let check = self
            .perform_check(session_token, browser_fp, hash, "{}", &cursor, debug_info)
            .await?;
