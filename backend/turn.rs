...
impl TurnAllocation {
    pub async fn connect(
        ...
        // [FOCSQ] Трафик к TURN-серверу не должен заворачиваться в туннель:
        // half-routes 0/1+128/1 бьют дефолт, без /32-exclude получается петля
        #[cfg(windows)]
        crate::tun_win::exclude_host_ip(server.ip());
        #[cfg(target_os = "linux")]
        crate::tun_linux::exclude_host_ip(server.ip());
