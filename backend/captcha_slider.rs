...
/// [FOCSQ] Траектория курсора для клика по чекбоксу. VK считает пустое
/// поведение (cursor=[]) признаком бота: слайдер с живой траекторией
/// проходил, чекбокс с [] стабильно ловил BOT.
pub(crate) fn build_checkbox_cursor() -> String {
    let mut random = rand::rng();
    let start_x = 16 + random.random_range(0..22);
    let start_y = 210 + random.random_range(0..70);
    let target_x = 52 + random.random_range(-6..7);
    let target_y = 26 + random.random_range(-4..5);
    let mut points: Vec<CursorPoint> = Vec::with_capacity(22);
    for _ in 0..random.random_range(1..4) {
        points.push(CursorPoint {
            x: start_x + random.random_range(-2..3),
            y: start_y + random.random_range(-2..3),
    ...
    let transit_steps = random.random_range(3..6);
    let control_x =
        f64::from(start_x + target_x) / 2.0 - f64::from(random.random_range(8..28));
    let control_y =
        f64::from(start_y + target_y) / 2.0 + f64::from(random.random_range(6..24));
    for index in 1..=transit_steps {
        let time = f64::from(index) / f64::from(transit_steps + 1);
        let x = (1.0 - time).powi(2) * f64::from(start_x)
            + 2.0 * time * (1.0 - time) * control_x
        ...
        let y = (1.0 - time).powi(2) * f64::from(start_y)
            + 2.0 * time * (1.0 - time) * control_y
            + time.powi(2) * f64::from(target_y);
        let jitter = ((1.0 - time) * 6.0) as i32 + 1;
        points.push(CursorPoint {
            x: x.round() as i32 + random.random_range(-jitter..=jitter),
            y: y.round() as i32 + random.random_range(-jitter..=jitter),
        });
    }
    for _ in 0..random.random_range(3..7) {
        points.push(CursorPoint {
            x: target_x + random.random_range(-2..3),
            y: target_y + random.random_range(-2..3),
        });
    }
    serde_json::to_string(&points).unwrap_or_else(|_| "[]".to_owned())
}
