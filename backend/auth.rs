...
impl VkAuth {
    ...
    pub async fn get_credentials(&self, link: &str, stream_id: usize) -> Result<TurnCredentials> {
        ...
        if self.mode.as_ref() == "vkcalls" {
            // [FOCSQ] Режим «Звонки» не использует капчу вовсе: fallback на
            // auto-captcha здесь вреден (Rust-солвер ловит BOT/ERROR_LIMIT,
            // а WBV без WebView мёртв). VK антифлудом режет параллельные
            // авторизации с одного IP — поэтому крутим звонки с растущим
            // бэкоффом, пока группа не получит креды. Отмена сессии
            // прерывает ожидание на стороне вызывающего (select! по cancel).
            let mut round: u32 = 0;
            loop {
                match self.vk_calls_with_retries(link, stream_id).await {
                    ...
                    Err(error) => {
                        ...
                        crate::log_error!(
                            "[STREAM {stream_id}] [VK Auth] Звонки не прошли ({error:#}) · повтор #{round} через {base}мс"
                        );
                        random_delay(base, base + 750).await;
                    }
                }
            }
        } else {
