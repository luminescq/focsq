...
impl<W: Write> Sink<W> {
    ...
    fn write_line(&mut self, value: &str, repetitions: usize) {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            // [FOCSQ] Строка собирается целиком и пишется ОДНИМ write_all:
            // каждый отдельный write() — отдельный чанк в колбэке хоста
            // (стрим Flutter-моста), и раньше " (x9" дробилось format!-ом
            // на фрагменты « (x» / «9» / «)» — в логе всплывали строки
            // «(x», «9», «)» вместо бейджа.
            let mut line = String::with_capacity(value.len() + 16);
            line.push_str(value);
            if repetitions > 1 {
                line.push_str(" (x");
                line.push_str(&repetitions.to_string());
                line.push(')');
            }
            line.push('\n');
...
pub struct Logger {
    shared: Arc<Shared>,
    // [FOCSQ] JoinHandle writer-потока нужен только мягкой остановке
    // (shutdown), отключённой в GUI-режиме — см. shutdown() ниже и
    // решение в lib.rs (конец run_client: логгер живёт между перезапусками).
    #[allow(dead_code)]
...
impl Logger {
    ...
    // [FOCSQ] Мягкая остановка writer-потока. В GUI-режиме отключена —
    // логгер живёт между перезапусками клиента (lib.rs, конец run_client);
    // сохранена для standalone-режима и прямого использования тестами.
    #[allow(dead_code)]
    ...
    // [FOCSQ] Используется только shutdown()/flush(); в GUI-режиме обе
    // отключены — см. shutdown() выше.
    #[allow(dead_code)]
...
// [FOCSQ] Изменено относительно оригинала csqtt: добавлен колбэк логов —
// ядро живёт в процессе хост-приложения (Flutter) и отдаёт строки через
// set_log_callback вместо печати только в stderr.
static LOG_CALLBACK: Mutex<Option<Box<dyn Fn(String) + Send + Sync>>> = Mutex::new(None);
...
impl std::io::Write for CallbackWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        ...
        // [FOCSQ] В GUI-приложении (WIN32, без консоли) GetStdHandle возвращает
        // NULL и первый же write падает с ERROR_INVALID_HANDLE — ошибка раньше
        // отключала весь Sink вместе с колбэком, и логи до Flutter не доходили.
        // Колбэк уже получил строку, поэтому запись в консоль — best effort.
        let _ = if self.is_stderr {
...
static LOGGER: LazyLock<Logger> = LazyLock::new(|| {
    Logger::with_writers(
        CallbackWriter { is_stderr: false },
        CallbackWriter { is_stderr: true },
        PROTOCOL_CAPACITY,
        DIAGNOSTIC_CAPACITY,
    )
});
...
// [FOCSQ] Точка входа мягкой остановки логгера. В GUI-режиме не зовётся
// (логгер живёт между перезапусками клиента — lib.rs, конец run_client);
// сохранена для standalone-режима и симметрии с Logger::shutdown.
#[allow(dead_code)]
